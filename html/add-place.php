...
$sql = "SELECT * FROM destinations WHERE id=$requested_id";
...
if (isset($_POST["submit"])) {
    ...
    $sql = "SELECT * FROM places WHERE id_destination='$requested_id' AND name='$name'";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) > 0) {
        $error["class"] = "alert-error";
        $error["message"] = "Submission failed: Place name already in use for this destination.";
    } else {
        $sql = "INSERT INTO places (place,name,img_url,link,id_destination) VALUES ('$activity','$name','$img','$link','$requested_id')";

        if (mysqli_query($conn, $sql)) {
            $error["class"] = "alert-success";
            $error["message"] = "Submission succeeded: Place was successfully added on this destination.";
        } else {
            $error["class"] = "alert-error";
...
<body>
    ...
    <main class="container-custom">
        <h1 class="text-center">Add A Place In <?php echo $current_destination["name"]; ?></h1>
        <form class="form-custom" action="" method="POST">
            <label for="activity">
                ...
                <select name="activity" id="activity">
                    ...
                    <option value="fun">Activities & Fun</option>
                </select>
            </label>
            <label for="name">
                Name:
                <input type="text" name="name" id="name" placeholder="Marriott Hotel" maxlength="60" required />
            </label>
            <label for="img">
                Image URL:
                <input type="text" name="img" id="img" placeholder="https://www..." maxlength="400" required />
            </label>
            <label for="link">
                External Link:
                <input type="text" name="link" id="link" placeholder="External Link with more information" maxlength="400" required />
            </label>
