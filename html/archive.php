...
$sql = "SELECT * FROM archives,posts,users WHERE archives.id_user=$user_id AND archives.id_post=posts.id AND posts.id_user=users.id";
...
<head>
    ...
    <link rel="stylesheet" href="style.css?dummy=w" />
...
<body>
    ...
    <main class="container-custom">
        <h1 class="text-center">My Archive</h1>
        <ul class="archived-posts-container">
            <?php
                foreach ($archives as $post) {
                    ...
                    <div class="archived-post">
                        ...
                        <div>
                            <a href="./user.php?id='.$post["id_user"].'" class="post-user">'.$post["username"].'</a>
                            <p class="post-body">'.$post["post"].'</p>
                            '.$photosString.'
                            
                            <a href="javascript:void(0)" onclick="archivePost(this, '.$post["id_post"].')" class="post-link">Unarchive</a> . <span style="color: #999;">'.date_format(date_create($post["created_at"]),"F d, Y").'</span>
                        </div>
                    </div>
                    ';
                }
                ...
                if (count($archives) == 0) {
                    echo "<p class='text-center'>You don't have any archived post.</p>";
                }
            ?>
        </ul>
    </main>
    ...
    <script>
        function archivePost(el, postId) {
            ...
            sendRequest(obj, '../server/archive_post.php');
