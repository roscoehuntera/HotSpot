...
$sql = "SELECT id_user_followed FROM follows WHERE id_user_follows=$user_id";
$result = mysqli_query($conn, $sql);

while ($row = mysqli_fetch_assoc($result)) {
    array_push($followingUsers, $row["id_user_followed"]);
}

$query = "(";

foreach ($followingUsers as $index=>$followingUserId) {
    $query .= "posts.id_user=$followingUserId";
    if ($index != count($followingUsers) - 1) {
        $query .= " OR ";
    }
}
...
if ($query != "()") {
    $sql = "SELECT * FROM users, destinations, posts WHERE posts.id_user=users.id AND destinations.id=posts.id_destination AND $query ORDER BY created_at DESC LIMIT 10";
    $result = mysqli_query($conn, $sql);
...
<head>
    ...
    <link rel="stylesheet" href="style.css?dummy=sas" />
        ...
        <div style="display: flex;">
            ...
            <div style="min-width: 400px; flex: 8 ;padding: 20px; padding-left: 0px;">
                ...
                <div id="postBar" style="margin-top:0;">
                    <h2 class="text-center">People you follow Posts</h2>
                    <?php
                        foreach ($allPosts as $post) {
                            ...
                            <div id= "post">
                                <div>
                                    <img src="'.$post["img_path"].'" style ="width: 75px; margin-right: 4px;">
                                </div>
                                <div>
                        ...
                        if (count($allPosts) == 0) {
                            echo "<p class='text-center'>There are no posts to show here.</p>";
                        }
                    ?>
                </div>
                <!--Posts-->
