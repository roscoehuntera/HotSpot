...
if (isset($_GET["place"])) {
    if ($_GET["place"] == "stay" || $_GET["place"] == "eat" || $_GET["place"] == "fun") {
        $activity = $_GET["place"];
        if ($_GET["place"] == "stay") {
            $title = "Places to stay in ";
        } else if ($_GET["place"] == "eat") {
            $title = "Places to eat in ";
        } else {
            $title = "Activities & fun in ";
        }
    }
} else {
    header('Location: destinations.php');
...
$sql = "SELECT * FROM places WHERE id_destination='$requested_id' AND place='$activity'";
$result = mysqli_query($conn, $sql);
...
<body>
    ...
    <main class="container-custom">
        <h1 class="text-center"><?php echo $title . $current_destination["name"]; ?></h1>
        <div class="all-destinations-container">
            <?php
                if (count($places) == 0) {
                    echo '<p>There are no places in this section. Please consider adding one.</p>';
                }
                foreach ($places as $place) {
                    echo '<a href="'.$place["link"].'" class="single-destination-card" target="_blank">
